...
int main(int argc, char **argv) {
    ...
    // генерация матриц (не учитываем в замере времени)
    srand(12345);                                        // фиксируем зерно, чтобы повторяемость
    ...
    double times_sum = 0.0;                              // аккумулируем суммарное время для усреднения
    for (int rep = 0; rep < repeats; ++rep) {
        // зануляем C перед каждым повтором (чтобы не влияли предыдущие результаты)
        for (size_t i = 0; i < total; ++i) C[i] = 0.0;

        // разбиение строк между потоками: базовое число строк + ровномерное распределение остатка
